...
def statistics_per_entity(dataset, entity_name, dir_path):
    ...
    # ----------------- Plots -----------------
    plot_entities = [other_entity_name + 's', 'papers', 'research_fields', 'research_problems']
    ...
    for plot_entity in plot_entities:
        ...
        Writer.write_png(plot, os.path.join(dir_path, '{}_distribution.png'.format(plot_entity)))

    distributions = scatters_plot(
        x_data=df['id'],
        y_data=[df['papers'], df['research_fields'], df['research_problems']],
        x_label= '{} (total={})'.format(entity_name + 's', statistics['number_of_' + entity_name + 's']),
        y_label='Value',
        legend_labels=['Papers (total={})'.format(statistics['number_of_papers']),
    ...
    Writer.write_png(distributions, os.path.join(dir_path, 'distributions.png'))
